...
! Per-step diagnostics (written by hydro_rad/blstep, read by snec timing)
 integer :: diag_hydro_rad_iters = 0   ! Newton-Raphson iterations in hydro_rad
 integer :: diag_scratch_count = 0     ! number of scratch retries this blstep
 logical :: diag_nickel_called = .false.
 logical :: diag_analysis_called = .false.
 integer :: diag_ni_eval_total = 0     ! total nickel_heating() calls
 integer :: diag_ni_eval_remap = 0     ! Ni evals forced by grid remap
...
!quantities related to the heating by radioactive Ni
 ...
 real*8 :: time_Ni
 real*8 :: time_Ni_last_fired = -1.0d30
...
!-------------------- grid ------------------------------------
  ...
  real*8 :: grid_update_interval_days = 0.0d0
  character(len=32) :: grid_mode = 'adaptive_runtime'
  character(len=256) :: grid_pattern_file = 'tables/GridPattern.dat'
  real*8 :: grid_surface_alpha = 5.0d0
...
!-------------------- evolution -------------------------------
  ...
  integer :: Ni_by_hand
  real*8 :: Ni_mass
  real*8 :: Ni_mix_fraction
  integer :: Ni_mix_kernel
  real*8 :: Ni_mix_component2_fraction = 0.0d0
  real*8 :: Ni_mix_component2_extent = -1.0d0
  real*8 :: Ni_period
  real*8 :: Ni_period_min_target
  real*8 :: Ni_period_max_target
  real*8 :: Ni_fractional_change_target
  integer :: Ni_quad_npoints = 150

  integer :: saha_ncomps

  logical :: boxcar_smoothing
  integer :: boxcar_smooth_ni = 1
  real*8 :: boxcar_nominal_mass_msun = 0.4d0
  integer :: boxcar_number_iterations = 4
  real*8 :: boxcar_ni_nominal_mass_msun = 0.4d0
  integer :: boxcar_ni_number_iterations = 4
...
!--------------------- timing -----------------------------------
  ...
  real*8 :: dtout_fast_target, dtout_mid_target, dtout_late_target
  real*8 :: dtout_scalar_fast_target, dtout_scalar_mid_target, dtout_scalar_late_target
  real*8 :: output_mid_transition_days, output_late_transition_days
...
!--------------------- Smooth Ni luminosity ---------------------
  ...
  integer :: smooth_ni_luminosity = 1        ! 0=hard zone cutoff, 1=smooth photosphere Ni contribution

!--------------------- Solver tolerances -------------------------

  real*8 :: epstol_hydro = 1.0d-4            ! Newton-Raphson convergence for hydro
  real*8 :: epstol_rad = 1.0d-4              ! Newton-Raphson convergence for hydro_rad
  integer :: itmax_hydro = 100               ! max iterations for hydro solver
  integer :: itmax_rad = 300                 ! max iterations for hydro_rad solver
...
!--------------------- Output mode ------------------------------------
  ...
  integer :: output_mode = 0                 ! 0=full output (default), 1=fitting-only minimal
