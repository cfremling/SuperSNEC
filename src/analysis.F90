subroutine analysis
  ...
  ! Fractional zone contribution for smooth Ni heating sum
  real*8 :: frac_ni_zone, dm_span, ni_zone_step, step_frac
...
!------------------- Calculate the observed luminosity ------------------------
  ...
  if(photosphere_fell_on_the_center.eq.0) then
     ...
     !Add fractional Ni contribution from the zone just below the photosphere.
     !mass_photo (interpolated at tau=2/3) lies between mass(index_photo-1)
     !and mass(index_photo).  The fraction of zone index_photo-1 above the
     !photosphere ramps smoothly from 0 to 1 as the photosphere recedes,
     !eliminating the step-like jump when index_photo decreases by 1.
     !Only applied when the potential step exceeds 2% of lum_observed —
     !at early times the zones are small and the hard cutoff is accurate.
     if (smooth_ni_luminosity .ge. 1 .and. index_photo .gt. 1) then
        ni_zone_step = Ni_energy_rate * &
             Ni_deposit_function(index_photo-1) * delta_mass(index_photo-1)
        step_frac = ni_zone_step / max(lum_observed, 1.0d30)
        if (step_frac .gt. 0.02d0) then
           dm_span = mass(index_photo) - mass(index_photo-1)
           if (dm_span .gt. 1.0d-20) then
              frac_ni_zone = (mass(index_photo) - mass_photo) / dm_span
              frac_ni_zone = max(0.0d0, min(1.0d0, frac_ni_zone))
           else
              frac_ni_zone = 0.0d0
           endif
           lum_observed = lum_observed + frac_ni_zone * ni_zone_step
        endif
     endif
  else
     lum_observed = lum(1) + &
