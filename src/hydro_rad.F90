subroutine hydro_rad
  ...
  ! EPSTOL and ITMAX now read from parameters module as epstol_rad, itmax_rad
...
!----------- update the temperature, pressure and internal energy -------------
  ...
  if(time.ge.time_Ni) then
      ! Skip actual Ni call if one was done very recently (e.g. regular
      ! cadence fired right before a grid remap forced another trigger).
      if (time - time_Ni_last_fired .gt. &
           0.5d0 * grid_update_interval_days * 86400.0d0) then
         call nickel_heating
         diag_nickel_called = .true.
         time_Ni_last_fired = time
         diag_ni_eval_total = diag_ni_eval_total + 1
      endif
      time_Ni = time + Ni_period
  endif
  ...
  do k=1, itmax_rad
    ...
    enddo
    if(delta_max.le.epstol_rad) then
       diag_hydro_rad_iters = k
       goto 101
    endif
  ...
  diag_hydro_rad_iters = itmax_rad
