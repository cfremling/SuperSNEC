...
\begin{abstract}
	We present {SuperSNEC}, a version of the SuperNova Explosion Code (SNEC) aimed at quickly producing large amounts of radiation-hydrodynamic models using low-zone-count simulations ($\sim100$ zones). We introduce adaptive gridding of the computational grid, which makes it possible to retain light-curve fidelity relative to a high-resolution SNEC baseline ($\sim1000$ zones), while offering very significant runtime improvements (${\sim}400\times$). SuperSNEC includes: (1) runtime adaptive gridding, (2) adaptive cadence control for radioactive $^{56}$Ni deposition, (3) optimized gamma-ray deposition with precomputed opacities and cache-efficient ray tracing, (4) runtime-optimized parameters and hydrodynamics-radiation solver tolerance thresholds for modeling stripped-envelope supernovae, (5) flexible parametrization and improved treatment of \Nif\ mixing with independent smoothing controls, and (6) a smooth photosphere luminosity correction that eliminates artifacts from discrete zone crossings during photosphere recession at low zone counts. We report speed versus accuracy trade-offs using a 100-zone configuration in SuperSNEC benchmarked against a 1000-zone reference SNEC run. We define baseline settings to be used for running a large parameter-space model grid of stripped-envelope supernova models as efficiently as possible without significantly affecting light-curve parameter inference after comparison to observations. Our optimized 100-zone baseline achieves an RMS magnitude residual of $0.024$~mag relative to a 1000-zone reference across all light-curve phases, at a runtime of ${\sim}1.7$~s per model. Light-curve parameters derived with SuperSNEC for SN~2011dh and SN~1993J agree with the literature consensus when using our baseline adaptive grid settings and 100 zones.
\end{abstract}
...
\subsubsection{Activation and remap scheduling}
\label{sec:remap_scheduling}
The frequency at which the grid is redistributed is controlled by the parameter \texttt{grid\_adaptive\_\-interval}. The default mode (\texttt{= 0}) attempts remaps at a fixed interval $\Delta t_{\mathrm{remap}}$ (\texttt{grid\_update\_\-interval\_days}; baseline 1.0~d). Several alternative scheduling strategies are available that reduce the remap frequency at late times, when the photosphere recedes slowly and the grid target changes little between updates.

At each scheduled check we rebuild the target grid from $q_{\mathrm{photo}}$. If the maximum boundary shift would be smaller than $10^{-4}$ of the total mass span, the remap is skipped to avoid unnecessary interpolation diffusion.

In the \textit{linear} mode (\texttt{= $k$}, for any positive integer $k \le 999$), the remap interval grows as $\Delta t_\mathrm{remap} = \max(\Delta t_\mathrm{base},\; t/k)$, where $\Delta t_\mathrm{base}$ is the base interval. Early times use the base interval; at late times the interval increases linearly with $t$.

In the \textit{photosphere-driven} mode (\texttt{= $-$1}), the remap interval is set by the photosphere recession rate,
\begin{equation}
\Delta t_\mathrm{remap} = \max\!\bigl(\Delta t_\mathrm{base},\; \min(q_\mathrm{thresh} / |\dot{q}_\mathrm{photo}|,\; 5\,\mathrm{d})\bigr),
\label{eq:photo_interval}
\end{equation}
where $q_\mathrm{photo}$ is the fractional mass coordinate of the photosphere ($\tau = 2/3$), $\dot{q}_\mathrm{photo}$ is its time derivative smoothed with an exponential filter ($\beta_\mathrm{s} = 0.5$), and $q_\mathrm{thresh} = 0.01$ is the maximum tolerated photosphere shift between remaps. A safety cap at $t/20$ prevents excessively long intervals if the rate estimate is noisy. An uncapped variant (\texttt{= $-$2}) omits this cap.

The \textit{early-dense} mode (\texttt{= $-$3}) uses a three-phase schedule: $\Delta t_\mathrm{remap} = 0.25$~d for $t < 3$~d, $0.5$~d for $3 \le t < 10$~d, and $\max(\Delta t_\mathrm{base},\, t/20)$ for $t \ge 10$~d, providing higher temporal resolution during the shock-cooling and rise phases. An empirical comparison of these modes across a range of base intervals is presented in Table~\ref{tab:remap_scheduling}.
...
We verified this by running the \texttt{adaptive\_\-runtime} mode at $N = 60$, $80$, $100$, $200$, $500$, and $1000$ zones with baseline parameters (Sect.~\ref{sec:baseline}). Table~\ref{tab:imax_scaling} summarizes the results. All six configurations complete successfully.

\vspace{-0.44cm}
\input{generated/table_imax_scaling.tex}

The RMS residuals improve steadily with increasing $N$: $\Delta m_\mathrm{all} = 0.047$~mag at $N = 60$, $0.027$~mag at $N = 100$, and $0.017$~mag at $N = 1000$. Reducing the zone count from $N = 1000$ to $N = 100$ cuts the runtime by ${\sim}95\%$, from $46.5$~s to $2.2$~s (at $n_\mathrm{quad}=150$; Appendix~\ref{sec:ni_quad}). The $N = 1000$ adaptive run itself completes in $47$~s, approximately $14\times$ faster than the unmodified SNEC reference run ($671$~s). The aggregate runtime optimizations that produce this overall speedup in SuperSNEC are detailed in Sect.~\ref{sec:speed_results}.
...
\label{eq:ni_cadence}
\end{equation}
where the $\mathrm{clip}(x, a, b) = \min(\max(x, a), b)$ function clamps its argument to the interval $[a, b]$, $f_{\mathrm{Ni}}$ is a user-specified fractional-change target (baseline $0.70$), and $\Delta t_{\mathrm{Ni,min}}$, $\Delta t_{\mathrm{Ni,max}}$ are the lower and upper bounds (baseline $5 \times 10^4$~s and $5.456 \times 10^5$~s). The result is quantized to the nearest integer number of hydrodynamic timesteps.

At early times, when $^{56}$Ni is decaying rapidly, this gives frequent updates (every $\sim0.6$~d). At late times, when the heating changes slowly on the $^{56}$Co timescale, updates become sparse (every $\sim6$~d). This avoids both the waste of recomputing a nearly unchanged deposition function and the inaccuracy of updating too rarely during the early Ni-dominated phase. The speed--accuracy trade-off for the cadence parameters is quantified in Sect.~\ref{sec:speed_results} (Table~\ref{tab:ni_cadence_comparison}).

In addition to the cadence optimization, we apply two micro-optimizations to the gamma-ray deposition integral itself. First, the product $\kappa_\gamma(i) = y_e(i) \times 0.06 \times \rho(i)$ (the gamma-ray opacity per zone) is precomputed into an array before the integration loops, eliminating redundant multiplications in the innermost loop. Second, the radial zone lookup, which in the original SNEC uses a binary search (bisection) for every integration point, is replaced with a sequential hunt: because successive ray-integration points move monotonically inward, the search starts from the previous result and steps forward or backward as needed. Together these changes reduce the per-call cost of \texttt{nickel\_heating()} by ${\sim}16\%$ at $N=100$, with zero change in the computed deposition function.

\subsection{Solver tolerance relaxation}
\label{sec:solver}

The implicit hydrodynamic and radiation-hydrodynamic solvers in SNEC (\texttt{hydro.F90} and \texttt{hydro\_rad.F90}) use Newton--Raphson iteration to converge the coupled temperature--pressure--velocity state at each timestep. Convergence is declared when the maximum relative temperature change across all zones falls below a tolerance $\mathrm{EPSTOL}$, subject to a maximum iteration count $\mathrm{ITMAX}$. In the original SNEC, $\mathrm{EPSTOL}$ is hard-coded to $10^{-7}$ in both solvers, and $\mathrm{ITMAX}$ is likewise a compiled constant; changing any of these values requires editing the source and recompiling. SuperSNEC promotes all four settings to runtime parameters (\texttt{epstol\_hydro}, \texttt{epstol\_rad}, \texttt{itmax\_hydro}, \texttt{itmax\_rad}), read from the configuration file, enabling systematic benchmarking without recompilation.
...
%% [CF: Multi-temperature filtered LC section moved to follow-up spectral synthesis paper]
%\subsection{Multi-temperature filtered light curves}
%\label{sec:multit}
%
%In the original SNEC, filtered (color) magnitudes are computed from a single effective temperature at the photosphere. The procedure is: (1) find the photosphere (the radius $R_{\mathrm{photo}}$ where the radial optical depth $\tau = 2/3$), (2) compute the effective temperature $T_{\mathrm{eff}} = (L_{\mathrm{photo}} / 4\pi\sigma_{\mathrm{SB}} R_{\mathrm{photo}}^2)^{1/4}$, where $L_{\mathrm{photo}}$ is the luminosity at the photosphere and $\sigma_{\mathrm{SB}}$ is the Stefan--Boltzmann constant, (3) look up bolometric corrections $\mathrm{BC}_b(T_{\mathrm{eff}})$ for each filter $b$ from a precomputed table (\texttt{BolCorr.dat}), and (4) compute apparent magnitudes $m_b = M_{\mathrm{bol},\odot} - \mathrm{BC}_b - 2.5\log_{10}(L_{\mathrm{photo}} / \Lsun)$, where $M_{\mathrm{bol},\odot}$ is the solar absolute bolometric magnitude and $\Lsun$ is the solar luminosity. All color information thus comes from a single temperature.
%
%This single-temperature approach has a well-known limitation: supernova ejecta are not blackbodies. Different wavelengths have different opacities, and therefore ``see'' different depths into the ejecta, each with a different local temperature \citep{Eastman1996,Dessart2005}. Blue photons are absorbed more readily than red ones (higher bound-free and line opacity at shorter wavelengths), so the effective photosphere for blue light lies at a larger radius (and lower temperature) than for red light. This effect becomes increasingly important as the ejecta expand and the photosphere recedes, since the temperature gradient steepens across the emitting region.
%
%We introduce a multi-temperature method that accounts for this wavelength-dependent depth of emission. The method uses the same bolometric correction table as the original SNEC (\texttt{BolCorr.dat}), but applies it locally at each radial zone rather than at a single photospheric temperature. The procedure, implemented in \texttt{snec\_gri\_multit.F90}, works as follows.
%
%For each radial zone $j$ with temperature $T_j$ and grey optical depth $\tau_j$ (measured from zone center to the surface), we define filter-dependent optical depths
%\begin{equation}
%\tau_{b,j} = f_b \times \tau_j,
%\end{equation}
%where $f_b$ is a scaling factor that encodes the relative opacity at the effective wavelength of each filter: $f_u = 1.3$, $f_g = 1.2$, $f_r = 1.0$ (the grey reference), $f_i = 0.8$, and $f_z = 0.7$. These factors reflect the well-established wavelength dependence of bound-free and line opacity in SN ejecta at photospheric temperatures ($T \sim 5{,}000$--$15{,}000$~K), where iron-group line blanketing strongly enhances the blue/UV opacity relative to red wavelengths \citep{PintoEastman2000}. At these temperatures, Rosseland mean opacities vary by factors of $\sim2$--$5$ across the optical, and the adopted range ($f_z/f_u \approx 0.5$) is consistent with this order of magnitude. The $r$-band is taken as the grey reference ($f_r = 1$) and the remaining factors are linearly interpolated. These values are first-order estimates; in principle, band-averaged opacity ratios could be refined by calibration against detailed non-LTE spectral synthesis calculations or observed SN color curves (e.g., SN~1993J).
%
%For each zone, we compute an escape weight per band:
%\begin{equation}
%w_{b,j} = e^{-\tau_{b,j+1}} - e^{-\tau_{b,j}},
%\end{equation}
%where $\tau_{b,j}$ and $\tau_{b,j+1}$ are the filter-scaled optical depths at the inner and outer boundaries of shell $j$, respectively. This quantity represents the fraction of emission from shell $j$ that escapes to the observer in band $b$. Deep zones with $\tau \gg 1$ have $w \approx 0$ (their emission is absorbed); surface zones with $\tau \ll 1$ also have $w \approx 0$ (they contain little emitting material). The peak contribution comes from zones near $\tau_b \sim 1$, the effective photosphere for that band.
%
%At each zone's local temperature, we look up the bolometric correction $\mathrm{BC}_b(T_j)$ and convert it to a band-to-bolometric luminosity fraction:
%\begin{equation}
%F_{b,j} = 10^{0.4 \times \mathrm{BC}_b(T_j)}.
%\end{equation}
%This quantity tells us what fraction of the bolometric luminosity emerges in band $b$ for a blackbody at temperature $T_j$.
%
%We then compute a weighted mean of this fraction across all zones:
%\begin{equation}
%\langle F_b \rangle = \frac{\sum_j r_j^2\, w_{b,j}\, F_{b,j}}{\sum_j r_j^2\, w_{b,j}},
%\end{equation}
%where $r_j$ is the radius of zone $j$ and the $r_j^2$ weighting accounts for the emitting area of each shell. The final band luminosity is
%\begin{equation}
%L_b = L_{\mathrm{bol}} \times \langle F_b \rangle,
%\end{equation}
%where $L_{\mathrm{bol}}$ is the total observed bolometric luminosity (including the Ni contribution above the photosphere).
%
%The key physical consequence is that each band naturally ``sees'' a different effective temperature: bluer bands have higher $\tau_b$ and therefore receive more emission from hotter, deeper layers, while redder bands probe cooler, more superficial layers. This produces more realistic color evolution than the single-$T_{\mathrm{eff}}$ method, particularly at late times when the temperature gradient across the emitting region is steep.
%
%We deliberately do not enforce $\sum_b L_b = L_{\mathrm{bol}}$: the five $ugriz$ bands cover only part of the electromagnetic spectrum, and the fractions $F_b$ are independent. The multi-temperature magnitudes are written to a separate file (\texttt{magnitudes\_ugriz\_multit.dat}) alongside the legacy single-temperature magnitudes (\texttt{magnitudes.dat}), so both methods are available for comparison.
...
\caption{Bolometric light-curve comparison of five configurations against a 1000-zone SNEC reference (black dashed line). \textit{Top panels:} luminosity; \textit{bottom panels:} magnitude residuals $\Delta m = -2.5\log_{10}(L/L_\mathrm{ref})$, with the green band marking $\pm0.1$~mag. \textit{Left:} full evolution (0--58~d); \textit{right:} early-time zoom (0--3~d). The original SNEC code at $N=100$ (purple dotted; $\Delta m_\mathrm{all}=0.161$~mag) diverges strongly at $t\lesssim0.5$~d and $t\gtrsim10$~d. SuperSNEC with the same fixed computational grid (orange dot-dashed) improves via adaptive \Nif\ cadence and solver tuning. The adaptive-runtime grid brings a further factor-of-three improvement: the baseline ($N=100$, $n_\mathrm{quad}=70$; blue solid line) and fast ($N=60$, $n_\mathrm{quad}=50$; red solid line) configurations both remain well within 0.1~mag across all phases. The SuperSNEC fixed-grid model uses $f_\mathrm{Ni}=0.10$; the baseline uses $f_\mathrm{Ni}=0.70$, $n_\mathrm{quad}=70$; the fast model uses $f_\mathrm{Ni}=1.00$, $n_\mathrm{quad}=50$. All three share $\alpha_\mathrm{surf}=7$, $t_\mathrm{relax}=5$~d, $\Delta t_{\mathrm{Ni,min}}=5\times10^4$~s, $\Delta t_{\mathrm{Ni,max}}=5.456\times10^5$~s. The original SNEC uses $\Delta t_{\mathrm{Ni,min}}=5\times10^4$~s with no adaptive cadence.}
...
\item Grid update cadence: $\Delta t_{\mathrm{remap}}$ (Appendix~\ref{sec:grid_cadence_sweep}).
...
Combined with zone reduction ($1000 \to 100$ zones) and the code updates described in Sect.~\ref{sec:code_updates}, these parameter optimizations compound into a total speedup of ${\sim}400\times$ relative to a 1000-zone SNEC reference run. Table~\ref{tab:cumulative_speedup} demonstrates this by applying each optimization successively: each row enables one additional change on top of all previous rows. The ``Cumulative'' column gives the total speedup factor relative to the SNEC reference; the ``Marginal'' column shows the incremental gain from that step alone. %All entries are measured wall-clock times from the benchmark script (\texttt{Analysis/scripts/run\_cumulative\_speedup.py}).

\vspace{-0.84cm}
\input{generated/table_cumulative_speedup.tex}

The dominant factor is zone reduction, which alone provides a ${\sim}60\times$ speedup. Compiler optimization (\texttt{-Ofast}) adds another ${\sim}1.2\times$. Switching to the SuperSNEC codebase, which includes the diagnostic cadence optimization (Sect.~\ref{sec:cadence}) and composition handling improvements (Sect.~\ref{sec:ni_mixing}), does not measurably change runtime at these settings (the solver dominates at $\mathrm{EPSTOL}=10^{-7}$ with full quadrature) but improves accuracy from 0.167 to 0.074~mag RMS. Relaxing the solver tolerance from $10^{-7}$ to $10^{-4}$ (Sect.~\ref{sec:solver}) provides ${\sim}1.2\times$, and the Ni ray-tracing optimization (Sect.~\ref{sec:ni_cadence}) yields a further ${\sim}2.4\times$. The adaptive Ni cadence and quadrature reduction collectively add another ${\sim}1.6\times$. Finally, enabling the adaptive grid mode (Sect.~\ref{sec:gridding}) with aggressive Ni cadence ($f_{\mathrm{Ni}} = 0.70$, safe in this mode because remap-forced updates dominate) does not change runtime but is the key enabler that makes 100 zones \emph{accurate}, reducing RMS from ${\sim}0.08$~mag (fixed grid) to ${\sim}0.024$~mag. If a somewhat higher RMS (${\sim}0.054$~mag) can be tolerated, another ${\sim}2\times$ speedup can be achieved with 60 zones, $n_\mathrm{quad} = 50$, and $f_{\mathrm{Ni}} = 1.00$ (``fast'' mode).

For large model-grid production runs aimed at LC fitting, the minimal output mode (Sects.~\ref{sec:output_mode} and~\ref{sec:output_cadence_speed}) provides an additional ${\sim}1.2\times$ speedup by writing only the 5 output files needed by LC fitting codes instead of the standard 61 SNEC output files. This reduces the baseline runtime from $1.7$~s to ${\sim}1.4$~s per model (${\sim}480\times$ faster than the SNEC reference run), or ${\sim}0.6$~s (${\sim}1000\times$) with the fast configuration. A million-model parameter-space sweep can thus be completed in ${\sim}16$~d of single-core wall-clock time with the baseline, or ${\sim}7$~d with the fast configuration. Because each model is independent and its runtime does not saturate a single core, trivial parallelism (launching independent SNEC instances) scales near-linearly with available cores: on a 16-core workstation, the same million-model grid should complete in ${\sim}1$~d (baseline) or ${\sim}10$~h (fast).
...
\item $^{56}$Ni cadence: $\Delta t_{\mathrm{Ni,min}} = 5.0\times10^4$~s, $\Delta t_{\mathrm{Ni,max}} = 5.456\times10^5$~s, $f_{\mathrm{Ni}} = 0.70$.
\item Grid update cadence: $\Delta t_{\mathrm{remap}} = 1.0$~d, fixed scheduling (\texttt{grid\_adaptive\_\-interval} $= 0$; Sect.~\ref{sec:remap_scheduling}).
...
\item Ni luminosity smoothing: \texttt{smooth\_ni\_\-luminosity} = 1 (on; Sect.~\ref{sec:ni_smoothing}).
\item Output mode: \texttt{output\_mode} = 0 (full; Sect.~\ref{sec:output_mode}). For production grid runs, \texttt{output\_mode\,=\,1} (minimal) is recommended.
\end{itemize}
With these settings the baseline achieves $\Delta m_\mathrm{all} = 0.024$~mag at $N = 100$ (Figure~\ref{fig:adaptive_runtime}); the corresponding runtimes and cumulative speedup factors are summarized in Table~\ref{tab:cumulative_speedup}.

\begin{figure*}[t!]
\centering
\includegraphics[width=\linewidth]{../Analysis/figures/lc_comparison_sne.pdf}
\caption{Bolometric light-curve (left) and Fe\,\textsc{ii}\,$\lambda5169$ photospheric velocity (right) comparison of two $N$=100 SuperSNEC models to SN~2011dh (top) and SN~1993J (bottom). Observational data: SN~2011dh bolometric LC from \citet{Ergon2014,Ergon2015}, velocities from \citet{Marion2014}; SN~1993J bolometric LC constructed from \citet{Richmond1994} photometry with UV/IR blackbody corrections, velocities measured from UNLV Supernova Archive spectra \citep{Barbon1995}.}
\label{fig:lc_comp}
\end{figure*}

\subsection{Test case: SN~2011dh and SN~1993J}
\label{sec:lc_comp}

As a demonstration that low-zone-count SuperSNEC models can reproduce observed stripped-envelope supernova light curves, we compare 100-zone models to two well-studied Type~IIb supernovae: SN~2011dh and SN~1993J. Table~\ref{tab:lc_comp} summarizes the progenitor and explosion parameters used in our models alongside literature values. 

These models were not found via thorough parameter space exploration or fitting, they are simply two hand-picked models based on two pre-supernova progenitor models drawn from a grid of binary MESA \citep{Paxton2011,Paxton2013} stellar-evolution models for SNe IIb (Hinds et al., in preparation). Each binary system (characterized by primary mass $M_1$, secondary mass $M_2$, and initial orbital period $P$) is evolved through Roche-lobe overflow and envelope stripping to the onset of core collapse, producing a He-star with a residual hydrogen envelope. The resulting pre-supernova star is described by its helium-core mass $M_\mathrm{He}$ (excluding the residual hydrogen envelope of mass $M_\mathrm{H}$), and pre-supernova radius $R$. The explosion is parametrized by the final kinetic energy $E_k$, synthesized $^{56}$Ni mass $M_{^{56}\mathrm{Ni}}$, and the ejecta mass $M_\mathrm{ej}$ (total pre-supernova mass minus the compact-remnant mass).

\paragraph{SN~2011dh}
The top panels of Figure~\ref{fig:lc_comp} compare the SuperSNEC model to the observed bolometric LC and photospheric expansion velocity evolution of SN~2011dh~\citep{Ergon2014}. The progenitor is a $M_1 = 16~\Msun$, $M_2 = 14~\Msun$ binary ($P = 160$~d) that produces a pre-supernova star with $M_\mathrm{He} = 4.28~\Msun$, $R = 100~\Rsun$, and $M_\mathrm{H} = 0.046~\Msun$. We explode this model with $E_k = 1.0 \times 10^{51}$~erg and $M_{^{56}\mathrm{Ni}} = 0.074~\Msun$, yielding an ejecta mass of $M_\mathrm{ej} = 2.45~\Msun$ (compact-remnant mass $1.98~\Msun$). The $^{56}$Ni mixing uses a step-function kernel with $q_\mathrm{mix} = 0.45$. The opacity floor is $0.03$~cm$^2$~g$^{-1}$ in the envelope and $0.12$~cm$^2$~g$^{-1}$ in the core. As shown in Table~\ref{tab:lc_comp}, our explosion parameters are broadly consistent with literature values.% \citep{Bersten2012,Ergon2014,Ergon2015,Marion2014,Jerkstrand2015}.

\paragraph{SN~1993J}
The bottom panels of Figure~\ref{fig:lc_comp} show an analogous comparison for SN~1993J, using a $M_1 = 15~\Msun$, $M_2 = 13~\Msun$ binary ($P = 240$~d) progenitor with $M_\mathrm{He} = 3.95~\Msun$, $R = 428~\Rsun$, and $M_\mathrm{H} = 0.10~\Msun$. We explode this model with $E_k = 1.35 \times 10^{51}$~erg and $M_{^{56}\mathrm{Ni}} = 0.10~\Msun$, yielding $M_\mathrm{ej} = 2.56~\Msun$ (compact-remnant mass $1.63~\Msun$). The $^{56}$Ni mixing uses $q_\mathrm{mix} = 0.17$ with a secondary component fraction of $0.25$ extending to $q = 1.0$. The opacity floor is $0.03$~cm$^2$~g$^{-1}$ in the envelope and $0.06$~cm$^2$~g$^{-1}$ in the core. These parameters fall within the literature range for SN~1993J (Table~\ref{tab:lc_comp}). The photospheric velocities are Fe\,\textsc{ii}\,$\lambda5169$ absorption-minimum measurements from spectra in the UNLV Supernova Spectrum Archive \citep{Barbon1995}, extracted by fitting the trough minimum in the 5000--5120~\AA\ window. The bolometric light curve we compare against is constructed from the \citet{Richmond1994} photometry with UV and IR blackbody corrections at $E(B-V) = 0.08$~mag.

Both SuperSNEC models used 100-zone computational grids with the adaptive runtime gridding settings described in Section~\ref{sec:gridding} and the working baseline parameters from Section~\ref{sec:baseline}. Despite the very low zone count, the models reproduce both the bolometric light curves and the early photospheric velocity evolution of each supernova, for explosion parameters consistent with the literature consensus. We do not attempt to derive uncertainty intervals for the explosion parameters here; this rather qualitative comparison is merely intended to validate the use of SuperSNEC for constructing large parameter-space model grids of stripped-envelope supernova models aimed at constraining explosion parameters via comparisons to observations.

\begin{deluxetable}{lcccc}
\setlength{\tabcolsep}{2pt}
\tablecaption{SuperSNEC model parameters compared to literature values for SN~2011dh and SN~1993J.\label{tab:lc_comp}}
\tablewidth{0pt}
\tablehead{
\colhead{Parameter} &
\multicolumn{2}{c}{SN~2011dh} &
\multicolumn{2}{c}{SN~1993J} \\
\colhead{} &
\colhead{This work} &
\colhead{Literature} &
\colhead{This work} &
...
\startdata
$R$ ($\Rsun$)                            & \textbf{100}   & 200--280\tablenotemark{a}    & \textbf{428}   & 430--620\tablenotemark{b} \\
$M_\mathrm{He}$ ($\Msun$)               & \textbf{4.28}  & 3.1--4.0\tablenotemark{a}    & \textbf{3.95}  & 3.0--5.0\tablenotemark{b} \\
$M_\mathrm{H}$ ($\Msun$)                & \textbf{0.046} & 0.01--0.10\tablenotemark{a}  & \textbf{0.10}  & 0.1--0.4\tablenotemark{b} \\
$M_\mathrm{ej}$ ($\Msun$)               & \textbf{2.45}  & 1.7--2.5\tablenotemark{a}    & \textbf{2.56}  & 1.9--3.5\tablenotemark{b} \\
$E_k$ ($10^{51}$~erg)                   & \textbf{1.0}   & 0.6--1.0\tablenotemark{a}    & \textbf{1.35}  & 1.0--1.6\tablenotemark{b} \\
$M_{^{56}\mathrm{Ni}}$ ($\Msun$)        & \textbf{0.074} & 0.06--0.09\tablenotemark{a}  & \textbf{0.10}  & 0.06--0.14\tablenotemark{b} \\
\enddata
...
\textit{Tridiagonal solver.} The implicit radiative-transfer step solves a tridiagonal linear system at each Newton--Raphson iteration using LAPACK's general banded solver \texttt{dgbsv}. Because the system is strictly tridiagonal ($k_l = k_u = 1$), the dedicated LAPACK routine \texttt{dgtsv}---or a hand-coded Thomas algorithm---would eliminate the overhead of general banded LU factorization and pivoting. At 100 zones the per-call cost is small, but the solve is executed ${\sim}3$--5 times per timestep (once per iteration in \texttt{hydro\_rad}), so the cumulative savings could be ${\sim}5$--$10\%$ of solver time.

\textit{Saha ionization caching.} The Paczynski equation of state calls the simplified Saha solver (\texttt{simple\_saha.F90}) at every zone, every Newton--Raphson iteration, every timestep. At $N=100$ zones with $\mathrm{EPSTOL}=10^{-4}$, the two implicit solvers (hydro and radiative transfer) each require ${\sim}3$--5 iterations per timestep, yielding ${\sim}600$--$1000$ Saha evaluations per timestep and ${\sim}10^6$ over a full simulation. Each evaluation solves a Newton--Raphson ionization balance for 3 species (\texttt{saha\_ncomps\,=\,3}); the remaining 16 composition species take the fully-ionized fast path. Reducing \texttt{saha\_ncomps} below 3 is not viable---it degrades light-curve quality significantly---while increasing it adds cost with no accuracy benefit, so the per-call work is already at its minimum. For zones deep in the optically thick interior, where temperature and density change by $\lesssim0.1\%$ per iteration, caching the Saha result and recomputing only when conditions exceed a threshold could skip ${\sim}50$--$70\%$ of the N-R solves, since the outer ${\sim}30$--$50$ zones (where the photosphere resides and conditions change rapidly) would still require full evaluation. Given that the EOS is a significant fraction of the per-timestep cost, this could yield a ${\sim}10$--$20\%$ overall speedup at $N=100$, with a larger fractional gain at higher $N$ where the Saha cost per call is unchanged but the number of interior zones grows.
...
Combined, the tridiagonal solver and Saha caching optimizations could plausibly yield a further ${\sim}1.3\times$ speedup; whether this is worth the added code complexity is left for future work.

\section{Reproducibility Notes}
Core analysis inputs and sweep snapshots are stored under \texttt{Analysis/\-results/}, and reproducibility scripts live in \texttt{Analysis/\-scripts/}. Paper tables are generated into \texttt{Paper/\-generated/} via \texttt{Analysis/\-scripts/\-generate\_\-paper\_tables.py}. The comprehensive Ni-cadence scan is produced by \texttt{Analysis/\-scripts/\-run\_ni\_cadence\_\-comprehensive.py} (results in \texttt{Analysis/\-results/\-ni\_cadence\_\-comprehensive\_v2/}), and the corresponding figure is generated by \texttt{Analysis/\-scripts/\-plot\_ni\_cadence\_\-tradeoff.py}. The early-excluded windowed residual metrics are generated by \texttt{Analysis/\-scripts/\-compute\_windowed\_\-lc\_metrics.py}. Extended solver-threshold diagnostics are produced with \texttt{Analysis/\-scripts/\-reproduce\_solver\_\-analysis.py}, which orchestrates parameter-sweep runs (\texttt{run\_legacy\_\-threshold\_limits.py}), figure generation (\texttt{plot\_solver\_\-threshold\_sweep.py}), and table regeneration.

\section{Paper Status}
This manuscript is an active draft. Sections, figures, and references will be expanded as optimization and validation proceed.

\begin{acknowledgments}
We acknowledge the excellent work by the original authors of SNEC that has made this work possible. C. Fremling is supported by NSF AST-2407588.
\end{acknowledgments}

\software{SNEC \citep{Morozova2015}, MESA \citep{Paxton2011, Paxton2013, Paxton2015, Paxton2018, Paxton2019}, GNU Fortran (GCC), gnuplot \citep{gnuplot}, NumPy \citep{Harris2020}, Matplotlib \citep{Hunter2007}, Python, Claude Opus 4.6 (Anthropic), Codex GPT-5.3, GPT 5.2 (OpenAI)}
...
This appendix presents the parameter sweep results that underlie the cumulative speedup of SuperSNEC  summarized in Sect.~\ref{sec:cumulative_speedup} and Table~\ref{tab:cumulative_speedup}.
...
Reducing $\mathrm{ITMAX}$ is counterproductive: a reduction from $(100,300)$ to $(5,15)$ nearly doubles the runtime due to the accumulation of scratched timesteps (timesteps where the solver fails to converge within $\mathrm{ITMAX}$ iterations, forcing the code to discard the step, halve the timestep, and retry) while the RMS residual remains unchanged. At $\mathrm{ITMAX}=(1,3)$ the simulation fails entirely. Moreover, the $\mathrm{ITMAX}$ scan was performed at the default $\mathrm{EPSTOL}=10^{-7}$; at a relaxed $\mathrm{EPSTOL}=10^{-4}$ the solver converges in fewer iterations and $\mathrm{ITMAX}$ is never reached (zero scratched steps), making it effectively inactive as a limiter. As such, we see no reason to raise $\mathrm{ITMAX}$ above its default, which could otherwise be motivated if scratched steps were a significant runtime contributor at the adopted $\mathrm{EPSTOL}$.

The dominant runtime lever is therefore $\mathrm{EPSTOL}$. Relaxing $\mathrm{EPSTOL}$ from $10^{-7}$ to $10^{-4}$ reduces the runtime from $4.6$~s to $1.7$~s with negligible impact on accuracy: the RMS change is $<0.005$~mag in all phase bins. Beyond $10^{-4}$, accuracy degrades progressively: at $\mathrm{EPSTOL} = 10^{-3}$ the 5--30~d RMS rises to $0.10$~mag, and at $\mathrm{EPSTOL} = 2 \times 10^{-3}$ it reaches $0.14$~mag. We adopt $\mathrm{EPSTOL}=10^{-4}$ with $\mathrm{ITMAX}(100,300)$ as the solver baseline for all subsequent parameter scans. This value sits at the accuracy knee: the RMS residual is flat from $10^{-7}$ to $10^{-4}$ but degrades sharply beyond, making $10^{-4}$ the most aggressive tolerance that does not sacrifice accuracy. The default $\mathrm{ITMAX}$ is retained because at $\mathrm{EPSTOL}=10^{-4}$ the solver always converges well within the iteration limit (zero scratched steps).

\subsection{Quadrature resolution}
\label{sec:ni_quad}

The gamma-ray deposition calculation in \texttt{nickel.F90} uses an $n_\mathrm{quad} \times n_\mathrm{quad}$ quadrature (\texttt{Ni\_quad\_\-npoints}) for the angular and radial integration of the optical depth to gamma rays. The original SNEC default of $n_\mathrm{quad} = 150$ ($22{,}500$ evaluations per zone) was chosen for 1000-zone models. We performed a convergence study at $N = 100$ zones, scanning $n_\mathrm{quad}$ from 30 to 150 (Table~\ref{tab:quad_convergence}). The RMS residual is essentially flat for $n_\mathrm{quad} \ge 50$ ($\Delta m_\mathrm{all} \approx 0.025$--$0.029$~mag), indicating that the quadrature is heavily over-resolved at 100 zones. We adopt $n_\mathrm{quad} = 70$ in as part of our baseline settings, which cuts the $N=100$ runtime from 3.5~s to 1.9~s relative to $n_\mathrm{quad}=150$ with no measurable accuracy loss. Reducing further to $n_\mathrm{quad} = 50$ yields an additional ${\sim}10\%$ speedup while remaining within 0.001~mag of the $n_\mathrm{quad}=70$ result. The quadrature flatness is a consequence of the coarse spatial grid: at 100 zones the optical-depth structure is smooth enough that fine angular/radial sampling adds no information. We choose $n_\mathrm{quad}=70$ rather than 50 as a modest safety margin for our recommended baseline settings, since the per-evaluation cost scales as $n_\mathrm{quad}^2$ and the difference between 70 and 50 is only ${\sim}10\%$ of total runtime. Users running at higher $N$ (e.g., 500--1000 zones for targeted follow-up) should increase $n_\mathrm{quad}$ toward the SNEC default of 150 to ensure convergence of the finer spatial structure.

\subsection{$^{56}$Ni deposition cadence}
\label{sec:ni_cadence_sweep}

The $^{56}$Ni heating calculation is the most expensive per-call operation in SNEC. In SuperSNEC its update cadence is controlled by three parameters: $\Delta t_{\mathrm{Ni,min}}$ (\texttt{Ni\_period}), $\Delta t_{\mathrm{Ni,max}}$ (\texttt{Ni\_period\_\-max}), and $f_{\mathrm{Ni}}$ (\texttt{Ni\_fractional\_\-change}). Because each grid remap in \texttt{adaptive\_\-runtime} mode forces a Ni recalculation (Sect.~\ref{sec:ni_remap_interaction}), the effective Ni update rate in that mode is dominated by the remap schedule rather than the cadence timer. This decouples the Ni cadence parameters from accuracy in \texttt{adaptive\_\-runtime} mode (especially at early times), but not in \texttt{legacy\_\-pattern} mode where no supplementary remap-forced updates occur.

Table~\ref{tab:ni_cadence_comparison} quantifies this difference by running matched Ni cadence settings across both grid modes. In \texttt{adaptive\_\-runtime} mode, the RMS is essentially independent of $f_{\mathrm{Ni}}$: even at $f_{\mathrm{Ni}} = 0.70$, the code performs 51 Ni evaluations (of which ${\sim}50$ are remap-forced), yielding $\Delta m_\mathrm{all} = 0.028$~mag. This insensitivity arises because the remap-forced updates are concentrated at early times, when the photosphere recedes rapidly and the grid is remapped most frequently, which is precisely when adequate Ni-heating temporal resolution matters most. In \texttt{legacy\_\-pattern} mode at the same $f_{\mathrm{Ni}} = 0.70$, only 14 Ni evaluations occur (no remap resets), and the RMS degrades to $0.17$~mag. The legacy grid requires $f_{\mathrm{Ni}} \leq 0.10$ (${\sim}94$--$101$ evals) to keep the 5--30~d bin below $0.1$~mag.

Table~\ref{tab:remap_scheduling} compares the four remap scheduling modes available in \texttt{adaptive\_\-runtime}, tested across base intervals from 0.5 to 3.0~d. Mode~0 uses the base interval as a fixed remap period. Modes~$-1$ and~$-2$ are photosphere-driven: the interval is set to $q_\mathrm{thresh}/|\dot{q}_\mathrm{photo}|$ (smoothed photosphere recession rate, $q_\mathrm{thresh}=0.01$), with mode~$-1$ adding a $t/20$ safety cap. Mode~$-3$ uses a fixed early-dense schedule (0.25~d for $t<3$~d, 0.5~d for 3--10~d, then $t/20$). At the 1~d baseline all four modes perform comparably. At longer base intervals (2--3~d), the photosphere-driven modes converge to the same behavior as the fixed mode because the base interval dominates over the physics-driven scheduling. Mode~$-3$ is the exception: its early-dense schedule operates independently of the base interval, maintaining RMS~$=0.080$~mag at a 3~d base interval where the fixed mode degrades to $0.125$~mag, with no runtime penalty. We nonetheless adopt mode~0 (fixed interval) as the baseline: at the 1.0~d baseline interval all four modes are equivalent, and mode~0 is the simplest and most predictable---the remap count is deterministic from the simulation duration and base interval. The adaptive scheduling modes ($-1$, $-2$, $-3$) offer runtime savings primarily through reduced remap counts at late times ($t \gtrsim 30$~d), but for evolution times $\lesssim 100$~d the late-time phase is short and the potential savings are small. Users running to significantly longer times (e.g., $200{+}$~d for nebular-phase studies) may benefit from mode~$-3$.

Table~\ref{tab:legacy_ni_cadence} shows the two-parameter legacy-pattern Ni-cadence sweep, varying $f_{\mathrm{Ni}}$ and $\Delta t_{\mathrm{Ni,max}}$ jointly at fixed $\Delta t_{\mathrm{Ni,min}} = 5\times10^4$~s. The fixed computational grid's tolerance is limited: at conservative cadence ($f_{\mathrm{Ni}} \leq 0.10$), the RMS is insensitive to $\Delta t_{\mathrm{Ni,max}}$ and stays below 0.1~mag. At aggressive cadence ($f_{\mathrm{Ni}} \geq 0.50$), accuracy degrades sharply, with the 5--30~d RMS exceeding 0.1~mag regardless of $\Delta t_{\mathrm{Ni,max}}$. Increasing $\Delta t_{\mathrm{Ni,max}}$ reduces Ni evaluations and runtime but does not compensate for aggressive $f_{\mathrm{Ni}}$.

We adopt $f_{\mathrm{Ni}} = 0.70$ with $\Delta t_{\mathrm{Ni,min}} = 5\times10^4$~s and $\Delta t_{\mathrm{Ni,max}} = 5.456\times10^5$~s as the Ni-cadence baseline. In \texttt{adaptive\_\-runtime} mode this is safe because the remap schedule dominates; in \texttt{legacy\_\-pattern} mode a more conservative $f_{\mathrm{Ni}} \leq 0.10$ would be required. With the 1.0~d fixed remap baseline, every remap forces a Ni recalculation (Sect.~\ref{sec:ni_remap_interaction}), so the effective Ni update cadence is ${\sim}1$~d regardless of $f_{\mathrm{Ni}}$ or the $\Delta t_{\mathrm{Ni}}$ bounds. The aggressive $f_{\mathrm{Ni}}=0.70$ therefore simply avoids unnecessary inter-remap recalculations without affecting accuracy. The $\Delta t_{\mathrm{Ni,min}}$ and $\Delta t_{\mathrm{Ni,max}}$ bounds have little practical effect in this mode but are retained at sensible values for users who may adopt longer remap intervals or switch to \texttt{legacy\_\-pattern} mode.

\subsection{Grid update cadence}
\label{sec:grid_cadence_sweep}
With the solver and Ni-cadence baselines locked, we performed a focused sweep of $\Delta t_{\mathrm{remap}}$ in \texttt{adaptive\_\-runtime} mode, varying only this single parameter across 12 values from 0.05~d to 3.0~d (plus adaptive gridding disabled). Disabling adaptive gridding entirely is fast (2.71~s) but fails both the early and mid-time bins (RMS~$=0.12$ and $0.29$~mag), confirming that adaptive gridding is essential for 100-zone models. At the other extreme, very frequent updates ($\leq0.1$~d) incur significant overhead (4--6~s) without improving accuracy, because the grid is being redistributed more often than the photosphere position changes appreciably.

The sweet spot lies between 0.7 and 1.5~d. In this range, runtimes are 2.3--2.6~s with all RMS bins well below 0.1~mag. Pushing beyond 2.0~d causes the 0--5~d and 5--30~d bins to cross the 0.1~mag threshold. We adopt $\Delta t_{\mathrm{remap}} = 1.0$~d as our baseline, which sits at the center of the 0.7--1.5~d safe range, giving equal margin on both sides of the accuracy boundary. The fixed-interval schedule is deterministic and reproducible. For evolution times $\lesssim 100$~d, the total overhead from $\lesssim 100$ remaps is small, so there is no compelling runtime reason to push toward longer intervals that would sacrifice early-time accuracy. Alternative scheduling modes are explored in Table~\ref{tab:remap_scheduling}.
...
\input{generated/table_solver_itmax_only.tex}
\input{generated/table_solver_epstol_only.tex}
\input{generated/table_quad_convergence.tex}
\input{generated/table_ni_cadence_comparison.tex}
\input{generated/table_remap_scheduling.tex}
\input{generated/table_legacy_ni_cadence.tex}
...
\texttt{grid\_adaptive\_interval}$^{\dagger}$ & Remap scheduling mode (Sect.~\ref{sec:remap_scheduling}) & 0 & 0 & \nodata \\
...
\texttt{Ni\_fractional\_change}$^{\dagger}$ & Fractional change trigger $f_{\mathrm{Ni}}$ & 0.25 & 0.70 & \nodata \\
...
\enddata
\tablecomments{The ``Default'' column lists the values shipped with SNEC~1.01. SuperSNEC-only parameters (marked $\dagger$) that have no SNEC equivalent show ``---''. The \texttt{Ni\_period\_max} default is $10\times\texttt{Ni\_period}$ when not explicitly set. SNEC~1.01 uses a single fixed output cadence; the three-band adaptive cadence (\texttt{dtout\_fast/mid} and transition days) is a SuperSNEC addition (Sect.~\ref{sec:cadence}).}
\end{deluxetable*}
