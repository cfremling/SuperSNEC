...
% R3 referee-round change marking (highlighting removed after review).
\newcommand{\rev}[1]{#1}
% Data-editor-round change marking. To remove all red after review,
% redefine as: \newcommand{\revd}[1]{#1}
...
\begin{abstract}
We present SuperSNEC, an accelerated version of the SuperNova Explosion Code (SNEC) designed for rapid production of large radiation-hydrodynamic model grids using low-zone-count simulations ($\sim100$ zones). The main advance is adaptive gridding of the computational grid, which preserves light-curve fidelity relative to a high-resolution SNEC baseline ($\sim1000$ zones) while delivering a runtime improvement of ${\sim}420\times$. SuperSNEC also includes solver optimizations, optimized radioactive-energy deposition and ray-tracing, and improved $^{56}$Ni mixing controls. We quantify the speed-accuracy trade-off for a 100-zone configuration against a 1000-zone reference and define baseline settings for efficient large-grid inference of stripped-envelope supernovae. Our optimized 100-zone setup achieves an RMS light-curve residual of $0.022$~mag relative to the 1000-zone reference, at a runtime of $<2$~seconds per model. Applied to SN~2011dh (Type~IIb), SN~1993J (Type~IIb), and SN~2020oi (Type~Ic), SuperSNEC recovers light-curve parameters consistent with the literature. In particular, SN~2020oi is well reproduced by a model powered by $^{56}$Ni decay and the standard hydrodynamic evolution, with no clear evidence that an additional power source such as CSM interaction or a central engine is required.
\end{abstract}
...
The dominant driver of the SNEC runtime is the computational-grid zone count, i.e., the number of zones used within each individual model\footnote{Throughout this paper, we separate a \emph{model grid} (an ensemble of SNEC runs in parameter space) and the \emph{computational grid} within each run (the Lagrangian mass grid discretized into zones).}. A default SNEC setup with 1000 zones takes on the order of 10 minutes to run on a current CPU\footnote{Runtime $671$~seconds on an Apple silicon M1 Pro CPU \rev{for the stripped-star model detailed in Sect.~\ref{sec:lc_validation}}. This and all other runtimes reported in this paper refer to simulations evolved to $\approx60$~d, unless otherwise noted.}. For robust unbiased light-curve parameter inference from an observed set of light curves, on the order of $1\times10^6$ models are needed to adequately map the required parameter space (Sect.~\ref{sec:grid}). This is only possible with SNEC \rev{on a personal computer} if the number of zones is reduced to $\sim100$, which reduces the runtime to $\sim10$~seconds on the same hardware. However, with such a low-zone-count computational grid the accuracy degrades, and the degradation can manifest either at early times or at late times depending on how the zones are distributed (Sect.~\ref{sec:gridding}). When modeling SNe that exhibit any kind of early light-curve feature ($\lesssim5$~d after explosion), such as the cooling phase following shock breakout in SNe IIb (e.g., \citealp{NakarPiro2014,Piro2015}), this is unacceptable since no tradeoff between early or late-time LC accuracy can be favorably made if the goal is parameter inference via comparison to observed LCs.
...
\end{equation}
Even a ``moderate'' choice of $N_i\sim6$--$10$ per dimension across six physical parameters implies $N_{\rm models}\sim(6\text{--}10)^6\approx5\times10^4$--$10^6$.  In practice, additional modeling systematics parameters, such as opacity-floor choices for the envelope and core used by the hydro/radiation solver, can be required to marginalize over modeling systematics. Allowing only $\sim3$--$4$ values for each of two such parameters multiplies the model-grid size by $\sim10$--$16$, pushing the total into the few$\times10^6$ regime. This motivates targeting $\sim10^6$ forward models as a practical baseline for unbiased SN~IIb light-curve inference, and it underscores why reducing per-model runtime (without sacrificing early- and late-time fidelity) is essential.  This ${\sim}10^6$ model count applies to a single pre-supernova progenitor structure from stellar evolution; a grid spanning multiple progenitor masses or binary configurations multiplies the total further.
...
\begin{figure*}[t!]
\centering
\includegraphics[width=\linewidth]{../Analysis/figures/grid_pattern_visualization.pdf}
\caption{Zone width $\Delta m$ vs.\ Lagrangian mass coordinate at three epochs for the $N=100$ adaptive grid (blue) and the fixed \texttt{GridPattern.dat} grid (red). Tick marks along the top (adaptive, blue) and bottom (fixed, red) edges show individual zone boundaries. Dashed vertical lines mark the photosphere. The adaptive grid tracks the photosphere inward \rev{by} shifting resolution from the outer layers to the interior as the ejecta evolve.}
\label{fig:grid_pattern}
\end{figure*}

\subsection{Runtime adaptive grid mode}
\label{sec:gridding}

In the original SNEC, the computational grid is set once at the start of the simulation and never changes. Two options are available: a uniform grid in mass coordinate, or a grid read from a file (\texttt{Grid\-Pattern.dat}\footnote{Throughout this paper, names typeset in \texttt{monospace} refer to parameter keywords in the SNEC configuration file or to filenames that ship with the SNEC distribution, unless otherwise noted. SNEC ships with a 1000-zone grid pattern; for lower zone counts we generate scaled patterns using the \texttt{grid\_setup.py} script included in the distribution.}). With 1000 zones this is adequate for the stripped-envelope progenitors considered in this work\footnote{The required zone count is progenitor-dependent: models with more extended envelopes (e.g., RSGs) may require significantly more zones. Users should verify convergence for their specific progenitor class by comparing runs at different zone counts.}, since there are enough zones to resolve both the surface layers, which are important during the first few days, when the photosphere sits near the stellar surface and the light curve is powered by shock-heated material cooling adiabatically, and the deep interior, which is important at later times, when the photosphere has receded inward and the light curve is powered by radioactive $^{56}$Ni decay heating diffusing outward.
...
\label{sec:grid_morphology}

At any given time the target grid has a piecewise structure: an inner region uniformly spaced in Lagrangian mass, followed by an outer region whose cell widths decrease geometrically toward the surface. This provides robust interior sampling while concentrating resolution \rev{near the surface, where the photosphere sits at early times}. This morphology is simpler than the fixed \texttt{GridPattern.dat} grid, yet it delivers substantially better light-curve fidelity at low zone counts because it places resolution where it is needed at each epoch (Figure~\ref{fig:grid_pattern}).  At $t=0$ the adaptive grid has a simple piecewise structure: uniform inner zones plus a geometric taper toward the surface. As the photosphere recedes inward ($t\approx10$~d, $t\approx25$~d), the adaptive grid \rev{relaxes its surface concentration and redistributes zones toward the interior}.
...
Temperature and internal energy are both remapped conservatively; thermodynamic consistency is restored by the subsequent implicit solve (which calls the EOS) on the remapped state. \rev{Quantitatively, at $N=100$ each remap conserves total mass and internal energy to machine precision, and changes the total kinetic energy (via the linear velocity interpolation) by a median $0.055\%$ per remap.}

\subsubsection{Composition handling}
For composition, we take advantage of the fact that the high-resolution progenitor profile ($\mathrm{few}\times1000$ zones from the stellar evolution\footnote{Any 1D stellar evolution code can provide progenitor profiles; e.g., MESA \citep{Paxton2011}, KEPLER \citep{WoosleyWeaver1995}, and \rev{FRANEC \citep{LimongiChieffi2018}}.}) is stored in memory at initialization. Rather than remapping the coarse 100-zone composition through successive interpolations, which would progressively smooth sharp interfaces, we re-map the stored high-resolution profile directly onto each new grid. This preserves the original composition structure regardless of how many grid updates have occurred. The mass-weighted conservative averaging preserves $\sum_i X_i = 1$ by construction; the code verifies closure after each mapping and aborts if it is violated beyond $10^{-8}$ after summing over all composition species. The stored high-resolution profile is also \rev{the input profile onto which our \Nif\ mixing prescription is applied} (Sect.~\ref{sec:ni_mixing}).
...
\subsubsection{Resolution robustness}
\label{sec:imax_scaling}

The adaptive gridding mode must function correctly at any zone count $N = \texttt{imax}$, not only at the baseline $N=100$ that we plan to use for producing large model grids (Sect.~\ref{sec:baseline}). This is important for two reasons: (1)~it allows users to increase resolution for targeted high-fidelity runs of individual models of interest identified from a fast model grid, and (2)~it validates that the adaptive scheme is numerically well-posed rather than tuned to a specific resolution. We emphasize that stability across $N$ does not imply resolution independence: the solution quality improves monotonically with increasing $N$ (Table~\ref{tab:imax_scaling}), as expected for a convergent numerical scheme.
...
Progenitor composition profiles from stellar evolution codes often contain sharp zone-to-zone discontinuities. \rev{Physically, these interfaces are not truly sharp: convective overshoot in the progenitor, and the Rayleigh--Taylor mixing and reverse shocks of the explosion, broaden them, so some smoothing before use in a radiation-hydrodynamics simulation  is beneficial  and better represents the real ejecta.} In SNEC, this is done via boxcar smoothing with a single hard-coded mass window and iteration count applied to all composition species simultaneously, coupling the $^{56}$Ni smoothing scale to that of the bulk composition. SuperSNEC exposes both the bulk smoothing parameters (\texttt{boxcar\_\-nominal\_\-mass\_\-msun}, \texttt{boxcar\_\-number\_\-iterations}) and independent $^{56}$Ni smoothing parameters (\texttt{boxcar\_ni\_\-nominal\_mass\_msun}, \texttt{boxcar\_ni\_\-number\_iterations}), allowing the user to smooth the bulk composition and the Ni profile on different scales. The independent smoothing of $^{56}$Ni coupled with the three available mixing kernels (Sect.~\ref{sec:ni_mixing}) enables a high level of flexibility in producing the final $^{56}$Ni profile of the ejecta. \rev{This independent control is physically motivated because $^{56}$Ni and the bulk composition are mixed by different processes: the bulk largely by the shock, and $^{56}$Ni by the instabilities that form the metal clumps (Sect.~\ref{sec:ni_mixing}). Being 1D, our scheme captures only the spherically-averaged radial extent of those clumps, not the angular structure of the true 3D mixing.}
...
\begin{figure*}[t!]
\centering
\includegraphics[width=0.98\linewidth]{../Analysis/figures/sedov_profiles_4panel.pdf}
\caption{Sedov blast-wave test at $t = 4$~s.  \rev{Each panel shows} one hydrodynamic variable: density ($\rho$), velocity ($v$), pressure ($p$), and specific internal energy ($U$, log scale).  Upper panels compare the analytical solution (black solid) to four numerical configurations: SNEC/SuperSNEC $N = 100$ on the static legacy grid (blue solid), SNEC/SuperSNEC $N = 1000$ on the legacy grid (orange solid), SuperSNEC $N = 100$ on the adaptive grid (green dashed), and SuperSNEC $N = 1000$ on the adaptive grid (red dashed).  Lower sub-panels show the fractional error $\delta = |(\mathrm{exact} - \mathrm{numerical}) / \mathrm{exact}|$.  The legacy-grid runs are labeled ``SNEC/SuperSNEC'' because both codes produce identical results to machine precision on the same grid.  The analytical solution is computed from the \citet{KammTimmes2007} parametric solver.  Setup: $\rho_0 = 1$~g\,cm$^{-3}$, $E_0 = 1$~erg, $\gamma = 1.4$, no radiation or gravity.}
\label{fig:sedov}
\end{figure*}
...
SuperSNEC makes no changes to the hydrodynamic equations, the radiation-hydrodynamic equations, or the Newton--Raphson iteration that integrates them.  The discretization, equation of state, opacity tables, and artificial viscosity are identical to SNEC~1.01; the only solver-level change is the tridiagonal solver replacement (Sect.~\ref{sec:solver}), which does not affect the solution to machine precision.  \rev{Validation of the core SNEC components, including comparison to the independent codes of \citet{Dessart2013} (CMFGEN) and \citet{Bersten2011}, was presented by \citet{Morozova2015} and carries over to SuperSNEC in full.}

To verify this explicitly, we perform the Sedov--Taylor blast-wave test \citep{Sedov1959}, which eliminates all progenitor-structure dependencies and provides an exact self-similar solution for comparison.  We adopt the setup bundled with the SNEC distribution: a uniform-density sphere ($\rho_0 = 1$~g\,cm$^{-3}$, $R = 2$~cm) with a 1-erg thermal bomb, an ideal gas ($\gamma = 1.4$), no radiation, and no gravity.  The exact solution is computed using the \citet{KammTimmes2007} parametric solver for all four hydrodynamic variables: density, pressure, specific internal energy, and material speed.


Figure~\ref{fig:sedov} compares the numerical profiles at $t = 4$~s to the analytical solution for four configurations: $N = 100$ and $N = 1000$ on both the legacy SNEC grid and the SuperSNEC adaptive grid ($\alpha_\mathrm{surf} = 7$).  We ran both the original SNEC~1.01 and SuperSNEC on the same legacy grids; the two codes produce identical results to machine precision, confirming that no SuperSNEC code modification affects the hydrodynamic stencil, time integration, or Newton--Raphson iteration.

All configurations agree well with the analytical solution across the full blast-wave profile, including the shocked region.  \rev{The largest deviations occur at $N = 100$ in the velocity field on the legacy fixed grid, which shows oscillations in the interior. The $N = 100$ adaptive grid is substantially smoother; small oscillations are visible only on close inspection (most clearly in the pressure). All of these features disappear at $N = 1000$ in both the fixed grid and adaptive modes.}

We also verified local energy conservation on the same stripped-star progenitor we use for light-curve validation (Sect.~\ref{sec:lc_validation}) by running both SNEC and SuperSNEC at $N = 100$ and $N = 1000$ and accumulating the per-step energy budget residual $|dE + (L + P\,dV - \dot{E}_\mathrm{Ni})\,dt|$ at the CO/He core boundary ($m \approx 3.2\,\Msun$) and at the photosphere ($m \approx 4.9\,\Msun$).  Over 0--58~days of evolution, both codes conserve energy locally to better than $0.074\%$ at $N = 1000$ at both boundaries, increasing to a maximum of $0.73\%$ at $N = 100$ in the adaptive mode due to the periodic zone remapping\rev{; the full dependence on zone count is given in Appendix~\ref{sec:energy_conservation} (Figure~\ref{fig:energy_conservation})}. Composition mass-fraction closure is verified to $10^{-8}$ after every grid remap (Sect.~\ref{sec:remap}).

\rev{Whether the small pressure oscillations seen at low $N$ in the adaptive mode, or the small energy conservation violation is important or can be ignored for a specific use case of SuperSNEC should be evaluated on a case by case basis. For our goal of bolometric light-curve fitting to derive supernova explosion parameters, the introduced errors on the light curve appear to be lower than typical photometric errors and therefore their impact is negligible (Sect.~\ref{sec:adaptive_validation}).}

\subsection{Light-curve comparison}
\label{sec:lc_validation}
To validate our runtime oriented optimizations and the adaptive runtime mode, we use comparisons to bolometric light curves from a 1000-zone reference SNEC run with default SNEC settings ($\mathrm{EPSTOL}=10^{-7}$, $\mathrm{ITMAX}(100,300)$, fixed computational grid from \texttt{Grid\-Pattern.dat}). The presupernova model is the stripped-envelope helium star from \citet{PiroMorozova2014,Morozova2015}, generated with the stellar evolution code MESA \citep{Paxton2011} from a $15\,\Msun$ zero-age main-sequence progenitor whose hydrogen envelope was removed during binary interaction. This model ships with the SNEC distribution; any presupernova profile in the same format (radial structure plus isotopic composition) is compatible with SuperSNEC. We use the thermal bomb energy injection mode throughout this work, since the final kinetic energy is a direct input parameter, unlike the piston mode where it must be inferred from piston velocity and timing. The reference runtime is $671$~seconds on an Apple Silicon M1~Pro CPU.
...
All three SuperSNEC models used 100-zone computational grids with the adaptive runtime gridding settings described in Section~\ref{sec:gridding} and the working baseline parameters from Section~\ref{sec:baseline} (Table~\ref{tab:parameters}). The models reproduce the bolometric light curves and photospheric velocity evolution of each supernova, for explosion parameters broadly consistent with the literature values (Table~\ref{tab:lc_comp}).\footnote{SNEC and SuperSNEC can also output broadband magnitudes via LTE blackbody bolometric corrections, but these are fundamentally inadequate for SE~SNe, where non-thermal excitation and NLTE effects become critical already around peak light.  We find up to factor-of-two errors in inferred $^{56}$Ni and ejecta mass when fitting SNEC multi-band lightcurve models to SE~SNe and comparing to parameters derived from bolometric fitting.}
...
The SuperSNEC source code, analysis scripts, and all data needed to reproduce the tables and figures in this paper are publicly available at \url{https://github.com/cfremling/SuperSNEC}. \revd{A frozen snapshot of the exact version used in this work (v1.0.0) is archived on Zenodo \citep[DOI: 10.5281/zenodo.22237491]{supersnec_zenodo}.} The repository includes the complete Fortran source, a ready-to-use parameter file, and progenitor stellar profiles. Building the code requires only a Fortran compiler (e.g., \texttt{gfortran}) and a LAPACK library; compilation is performed with a single \texttt{make} command.
...
\software{\revd{SuperSNEC v1.0.0 \citep{supersnec_zenodo},} SNEC \citep{Morozova2015}, MESA \citep{Paxton2011, Paxton2013, Paxton2015, Paxton2018, Paxton2019}, GNU Fortran (GCC), gnuplot \citep{gnuplot}, NumPy \citep{Harris2020}, Matplotlib \citep{Hunter2007}, Python, Claude Opus 4.6 \citep{ClaudeOpus46}, GPT-5 \citep{GPT5SystemCard}, Codex GPT-5.3 \citep{GPT53Codex}}
...
% Grid pattern visualization moved to Sect.~\ref{sec:grid_morphology}
\label{sec:grid_pattern_viz}
...
\begin{figure*}[t!]
\centering
\includegraphics[width=0.95\linewidth]{../Analysis/figures/local_energy_conservation.pdf}
\caption{\rev{Cumulative local energy-conservation residual at the CO/He core boundary (left) and the photosphere shell (right) over the $0$--$58$~d evolution of the stripped-star model. Solid curves are the fixed computational grid, on which SNEC and SuperSNEC agree to machine precision (Sect.~\ref{sec:verification}); dashed curves are the SuperSNEC adaptive grid at $N = 100$, $200$, $500$, and $1000$. On the fixed grid the residual stays below $0.1\%$ and is essentially independent of zone count, while the adaptive remap adds a residual that decreases monotonically with zone count, reaching the fixed-grid level by $N \approx 500$.}}
\label{fig:energy_conservation}
\end{figure*}

\rev{We extend the local energy-conservation test described in Sect.~\ref{sec:verification} (the cumulative per-step budget residual at the CO/He core boundary and the photosphere shell, normalized by the enclosed energy) to a range of zone counts, evaluating it for the original SNEC and for SuperSNEC on both the fixed legacy grid and the adaptive grid at $N = 100$, $200$, $500$, and $1000$.}

\rev{Figure~\ref{fig:energy_conservation} shows the result. On the fixed grid, SNEC and SuperSNEC are indistinguishable in this diagnostic, with maximum residuals of $0.077\%$ (CO/He) and $0.013\%$ (photosphere) at $N = 100$, essentially unchanged at $N = 1000$ ($0.068\%$ and $0.015\%$). The adaptive remap raises the residual at low zone count, but it decreases monotonically as $N$ increases: at the CO/He boundary and the photosphere respectively, the maximum residual is $0.55\%$ and $0.73\%$ at $N = 100$, $0.23\%$ and $0.23\%$ at $N = 200$, $0.086\%$ and $0.050\%$ at $N = 500$, and $0.074\%$ and $0.023\%$ at $N = 1000$, converging to the fixed-grid level by $N \approx 500$.}
