...
def build_configs() -> List[Config]:
    ...
    configs = [
        ...
        Config(
            ...
            description="+ precomputed kappa_gamma + sequential hunt",
            codebase="supersnec",
            param_overrides=overlay(supersnec_legacy_base, {
                "epstol_hydro": "1.0d-4",
                "epstol_rad": "1.0d-4",
        ...
        # 8. + Adaptive grid mode + f_Ni=0.70 + three-band cadence
        #    (the full working baseline)
        Config(
            label="Adaptive grid + $f_{\\mathrm{Ni}}{=}0.70$",
            description="+ adaptive_runtime grid with f_Ni=0.70 + three-band cadence",
            codebase="supersnec",
        ...
        # 9. Fast mode: imax=60, f_Ni=1.0, quad=50, smooth_ni=1
        Config(
            label="Fast ($N{=}60$, $n_{quad}{=}50$, $f_{\\mathrm{Ni}}{=}1.0$)",
            description="imax=60, f_Ni=1.0, quad=50, smooth_ni=1 (maximum speed)",
            codebase="supersnec",
            param_overrides=overlay(overlay(supersnec_legacy_base, three_band_cadence), {
                "imax": "60",
                "epstol_hydro": "1.0d-4",
        ...
        Config(
            label="Fast",
            description="Fast (imax=60, f_Ni=1.0, quad=50) + output_mode=1",
            codebase="supersnec",
            param_overrides=overlay(overlay(supersnec_legacy_base, three_band_cadence), {
                "imax": "60",
                "epstol_hydro": "1.0d-4",
